...
export default function Ingredient(props) {
  ...
  //set up
  const { markStock, deleteCocktail, editCocktail } = props;
  const navigation = useNavigation();

  const handleCardPress = () => {
    navigation.navigate('CardView', { cocktailId: props.id });
  }
  ...
  const options = [
    ...
    onPress: () => redirectToEdit(props.id)},
    {
      ...
      onPress: () => deleteCocktail(props.id)
  },
    {
      title: stockOption,
  ...
  redirectToEdit = (id) => {
    ...
    navigation.navigate('EditCocktail', { ingredientId: id });
  };
